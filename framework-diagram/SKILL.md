...
6. **节点底色只表示职责或技术层级。** 维护一份配色语义源，同时生成每张 SVG
   顶部的实际用色/线型图例，以及每篇 HTML 文末的完整色块说明。颜色不表达执行
   顺序、成功/失败或数据格式。
7. **间距的含义要先看清**：用户说"间距过小"可能是
...
3. 在生成器中定义图（band → box → edge → text → finish），遵守 layout-spec；
   从统一配色表生成图内图例和 HTML 文末色块说明。
4. 运行生成器；**任何 WARN 都视为失败**。必须检查：外部资源、SVG XML、
   可访问性、重复 ID、文字溢出、连线穿框、非正交线段、画布越界、图例覆盖的
   实际 class，以及每篇 HTML 恰有一份完整色块说明。
...
每张 SVG 顶部自动生成图例，分“底色”和“连线”展示当前图实际使用项；不得在单图中
手写一份可能漂移的图例。完整规则见 `layout-spec.md` §5。
