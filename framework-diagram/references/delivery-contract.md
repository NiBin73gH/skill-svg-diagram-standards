...
- 每篇生成的 HTML 在 `</article>` 前恰有一个 `section.color-key`，包括没有流程图的页面。
- 色块说明列出 `layout-spec.md` §3 的完整节点类别，且从图内图例使用的同一配色
  语义数据生成。
- 固定说明颜色只表示职责/技术层级，不表示顺序、成功/失败或数据格式。
- 桌面端使用两列网格，窄屏切换为单列。
